...
use helix_event::{cancelable_future, TaskHandle};
use helix_stdx::{
...
fn local_search_markdown_section(cx: &mut Context, file_contents: String) {
    ...
    let mut max_lengths: [usize; 6] = [0, 0, 0, 0, 0, 0];

    let mut file_results: Vec<AlmostFileResult> = file_contents
        .lines()
        .enumerate()
        .filter_map(|(line_num, line)| {
            ...
            let mut chars = line_trim.chars();
            let level = if line_trim.starts_with('#') {
                chars.by_ref().take_while(|chr| chr == &'#').count()
            } else if line_trim == "---" {
                // we ensured the line is entirely ascii, therefore no multibyte chars
                line.as_bytes()
                    .chunks(whitespace_per_indent)
                    .take_while(|the| the.iter().all(|ch| *ch == b' ' || *ch == b'\t'))
                    .count()
                    + 1
            } else {
                0
            ...
            max_lengths[level - 1] = max_lengths[level - 1].max(title.chars().count());
            heading_stack[(level - 1)..].fill(None);
            heading_stack[level - 1] = Some(title);

            let parts = heading_stack
                .iter()
                .take(level)
                .flatten()
                .cloned()
    ...
    file_results.sort_by_key(|the| the.level);

    let file_results = file_results.into_iter().map(|container| {
        let heading = container
            .parts
            .iter()
            .enumerate()
            .map(|(index, part)| format!("{part:<padding$}", padding = max_lengths[index]))
            .collect::<Vec<_>>()
            .join(" > ");
        FileResult {
            line_num: container.line_num,
            heading,
        }
    });
...
fn local_search_section(cx: &mut Context) {
    ...
    let file_results: Vec<FileResult> = contents
        ...
        .filter_map(|(line_num, line)| {
            ...
            longest_title = longest_title.max(end - beg);
    ...
    let columns = [PickerColumn::new(
        ...
        |item: &FileResult, config: &LocalSearchData| {
            ...
            let longest_title = config.longest_title;
            ...
            Cell::from(Spans::from(vec![Span::raw(format!(
                "{:<longest_title$}",
                content
            ))]))
        },
...
fn ai_edit(cx: &mut Context) {
    ...
    // Helix represents a plain cursor as a one-grapheme block cursor. Keep
    // that character outside the replacement range so cursor edits insert
    // exactly at the cursor, while real selections are replaceable (including
    ...
    let prompt = Prompt::new(
        ...
        move |cx: &mut compositor::Context, input: &str, event: PromptEvent| {
            ...
            if input.trim().is_empty() {
                cx.editor.set_error("AI edit request cannot be empty");
                return;
            }

            let request = request.clone();
            let instruction = input.to_owned();
            cx.editor.set_status("AI edit: asking Codex...");
            cx.jobs.callback(async move {
                let replacement = run_codex_ai_edit(&request, &instruction).await?;
                Ok(Callback::EditorCompositor(Box::new(move |editor, _| {
                    apply_ai_edit(editor, request, replacement);
                })))
            });
        },
    );
...
async fn run_codex_request(
    ...
    cancellation: Option<&TaskHandle>,
    ...
    command
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::piped());
    let process = match cancellation {
        Some(cancellation) => cancelable_future(command.output(), cancellation).await,
        None => Some(command.output().await),
    }
    .transpose()
    .context("failed to start Codex; install it and run `codex login`")?;
    ...
    std::fs::read_to_string(&output)
        .context("Codex returned no response")
        .map(Some)
}
...
fn apply_ai_edit(editor: &mut Editor, request: AiEditRequest, replacement: String) {
    ...
    {
        editor.set_error("AI edit discarded because its view was closed or changed documents");
        return;
    }

    let view = view_mut!(editor, request.view_id);
    let doc = doc_mut!(editor, &request.doc_id);
    if doc.version() != request.version {
        editor
            .set_error("AI edit discarded because the document changed while Codex was responding");
        return;
    }

    // This deliberately uses the ordinary apply path, rather than a temporary
    // transaction. It emits didChange for both insertions and deletions, so
    // the language server stays in lockstep with the document.
    doc.append_changes_to_history(view);
    let transaction = Transaction::change(
        doc.text(),
        [(request.start, request.end, Some(replacement.into()))].into_iter(),
    );
...
fn request_ai_suggestion(cx: &mut Context, delay: Duration, show_status: bool) {
    ...
    cx.jobs.callback(async move {
        if !delay.is_zero()
            && cancelable_future(tokio::time::sleep(delay), &cancellation)
                .await
                .is_none()
        {
...
fn show_ai_suggestion(
    ...
    if let Some(editor_view) = compositor.find::<ui::EditorView>() {
        // Ghost text owns completion input while it is visible. Restoring a
        // temporary completion preview here keeps the document/LSP state real.
        let _ = editor_view.clear_completion(editor);
    }
    doc_mut!(editor, &request.doc_id).set_ai_suggestion(
        request.view_id,
...
pub(crate) fn ai_suggestion_active(compositor: &mut Compositor) -> bool {
    compositor
        .find_id::<AiSuggestion>(AI_SUGGESTION_ID)
        .is_some()
}
...
impl AiSuggestion {
    ...
    fn restore_completion(&self, editor: &mut Editor) {
        if !editor.tree.contains(self.view_id) || editor.tree.get(self.view_id).doc != self.doc_id {
            return;
        }
        let doc = &editor.documents[&self.doc_id];
        ...
        editor
            .handlers
            .trigger_completions(cursor, self.doc_id, self.view_id);
    }
    ...
    fn handle_event(
        ...
        match event {
            ...
            compositor::Event::Key(key) if *key == key!(Esc) || *key == ctrl!('c') => {
                ...
                self.restore_completion(cx.editor);
                compositor::EventResult::Consumed(Some(close()))
            }
...
fn flash_jump_overlays(editor: &mut Editor, matches: &[FlashJumpMatch]) -> Vec<(usize, char)> {
